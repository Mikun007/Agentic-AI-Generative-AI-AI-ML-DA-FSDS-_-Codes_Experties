...
@st.cache_data
def load_image():
    url = "https://transforms.stlzoo.org/production/animals/amur-tiger-01-01.jpg?w=1200&h=1200&auto=compress%2Cformat&fit=crop&dm=1658935145&s=95d03aceddd44dc8271beed46eae30bc"
    response = requests.get(url)
    return Image.open(BytesIO(response.content)).convert("RGB")

# Load and display image
elephant = load_image()
st.image(elephant, caption="Original Elephant Image")
...
with col1:
    st.image(red_img, caption="Red Channel")

with col2:
    st.image(green_img, caption="Green Channel")

with col3:
    st.image(blue_img, caption="Blue Channel")
